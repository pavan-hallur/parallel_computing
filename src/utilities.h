...
public:
    ...
    size_t getMNups(long updated_nodes) const
    {
        ...
        std::chrono::duration<double> diff = std::chrono::high_resolution_clock::now() - startNupsTime;
        return (size_t)((double)updated_nodes / diff.count() / 1E6);
    }

    size_t getAverageMNups(long updated_nodes) const
    {       
        std::chrono::duration<double> runtime = std::chrono::high_resolution_clock::now() - startRuntime; 
        return (size_t)((size_t) updated_nodes / runtime.count() / 1E6);
    }
